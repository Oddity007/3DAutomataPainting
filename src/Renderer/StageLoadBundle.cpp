...
void GX::Stage::loadBundle(uint16_t bundleIndex)
{
	...
	lua_getglobal(L, "EarthUpImageIndex");
	renderer.paintingData.earthUpImageIndex = lua_tonumber(L, 1);
	lua_pop(L, 1);
	
	lua_getglobal(L, "EarthSideImageIndex");
	renderer.paintingData.earthSideImageIndex = lua_tonumber(L, 1);
	lua_pop(L, 1);
	
	lua_getglobal(L, "EarthDownImageIndex");
	renderer.paintingData.earthDownImageIndex = lua_tonumber(L, 1);
	lua_pop(L, 1);
	
	lua_getglobal(L, "MetalImageIndex");
	renderer.paintingData.metalImageIndex = lua_tonumber(L, 1);
	lua_pop(L, 1);
